...
public class HBaseUtil {
     ...
     * @return
     */
    // 没有异常处理能力(业务能力)的时候需要将异常抛出而不是使用try catch
    ...
    public static void initNamespace(Configuration conf, String namespace) throws IOException {
        ...
        Admin admin = connection.getAdmin();
        // 创建命名空间 创建时间，作者
        NamespaceDescriptor nd = NamespaceDescriptor.create(namespace).addConfiguration("CREATE_TIME", String.valueOf(System.currentTimeMillis())).addConfiguration("AUTHOR", "LC").build();
    ...
    public static byte[][] genSplitKeys(int regions) {
        ...
        // 当前数据region个数不会超过2位数，所以region分区键格式化为两位数字所代表的字符串
        DecimalFormat df = new DecimalFormat("00");
        for (int i = 0; i < regions; i++) {
            keys[i] = df.format(i) + "|";
        }
        byte[][] splitKeys = new byte[regions][];
        // 排序 生成byte[][]类型的分区键的时候，必须保证分区键是有序的
     ...
     * @return
     */
    public static String genRowKey(String regionCode, String call1, System buildTime, String call2, String  flag, String duration) {
        // 字符串操作
    ...
    /**
     * 手机号: 18514587532
     * 通话建立时间: 2018-05-01 12:01:10 => 20180501120110
     * @param call1
     * @param buildTime
     * @param regions
     * @return
